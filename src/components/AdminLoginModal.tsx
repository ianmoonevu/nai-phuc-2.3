...
export const AdminLoginModal: React.FC<AdminLoginModalProps> = ({
  ...
  // Focus ID input on modal open & reset warnings
  useEffect(() => {
    if (isOpen) {
      setAdminId('');
      setPassword('');
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <h2 className="text-xl font-black text-[#00356a] tracking-tight">
              Admin Login
            </h2>
            <p className="text-xs text-[#00356a]/60">
              Sign in with administrative credentials to continue.
            </p>
          </div>
        </div>

        {/* Warning Alert Box when input is wrong */}
        {errorMessage && (
          <div
        ...
        <form onSubmit={handleSubmit} className="mt-5 space-y-4">
          ...
          <div>
            <label
              ...
              className="block text-[11px] font-bold uppercase tracking-wider text-[#00356a] mb-1.5"
            >
              Admin ID
            </label>
            <div className="relative">
              ...
              <input
                ...
                className={`w-full pl-10 pr-4 py-3 rounded-2xl text-xs font-semibold text-[#00356a] bg-[#f4f6f8] border transition-all focus:outline-none focus:ring-2 ${
                  errorMessage
                    ? 'border-rose-400 bg-rose-50/30 focus:ring-rose-400'
                    : 'border-[#dce0e6] shadow-bubble-inset focus:ring-[#00356a] focus:bg-white'
                }`}
              />
            </div>
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <label
                ...
                className="block text-[11px] font-bold uppercase tracking-wider text-[#00356a]"
              >
                Password
              </label>
              <span className="text-[10px] text-[#00356a]/50 font-medium">
                Case-sensitive
              </span>
            </div>
            <div className="relative">
              ...
              <input
                ...
                className={`w-full pl-10 pr-11 py-3 rounded-2xl text-xs font-semibold text-[#00356a] bg-[#f4f6f8] border transition-all focus:outline-none focus:ring-2 ${
                  errorMessage
                    ? 'border-rose-400 bg-rose-50/30 focus:ring-rose-400'
                    : 'border-[#dce0e6] shadow-bubble-inset focus:ring-[#00356a] focus:bg-white'
                }`}
              ...
              <button
                ...
                className="absolute inset-y-0 right-0 pr-3.5 flex items-center text-[#00356a]/40 hover:text-[#00356a] transition-colors cursor-pointer"
          ...
          <div className="pt-2 flex items-center justify-end gap-3">
            <button
              ...
              onClick={onClose}
              className="px-4 py-2.5 rounded-full text-xs font-bold text-[#00356a]/70 hover:text-[#00356a] hover:bg-[#f4f6f8] transition-all cursor-pointer"
            >
              Cancel
            </button>
        ...
        <div className="mt-6 pt-4 border-t border-[#f0f3f5] flex items-center justify-between text-[10px] text-[#00356a]/50">
          ...
          <span>Security Protocol v3.0</span>
        </div>
      </div>
    </div>
  );
