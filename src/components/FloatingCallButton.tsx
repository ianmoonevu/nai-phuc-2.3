...
export const FloatingCallButton: React.FC = () => {
  ...
  return (
    <div
      ...
      className="fixed bottom-4 left-3 sm:bottom-7 sm:left-6 z-40 flex flex-col items-start gap-2 font-sans select-none max-w-[calc(100vw-24px)]"
    >
      {/* Expandable Social Channels Mini-Dock */}
      {showSocialDock && (
        <div
          id="floating-social-mini-dock"
          className="bg-white/95 backdrop-blur-md rounded-2xl p-2.5 shadow-2xl border border-[#e2e6eb] flex flex-col gap-2 animate-in slide-in-from-bottom-2 duration-200 max-w-[280px] sm:max-w-xs"
        >
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            {social.zalo?.enabled !== false && social.zalo?.url && (
              <a
                ...
                className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-[#0068FF]/10 text-[#0068FF] hover:bg-[#0068FF] hover:text-white transition-all text-xs font-bold shadow-sm cursor-pointer group min-h-[38px]"
              >
            ...
            {social.facebook?.enabled !== false && social.facebook?.url && (
              <a
                ...
                className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-[#1877F2]/10 text-[#1877F2] hover:bg-[#1877F2] hover:text-white transition-all text-xs font-bold shadow-sm cursor-pointer group min-h-[38px]"
              >
            ...
            {social.linkedin?.enabled !== false && social.linkedin?.url && (
              <a
                ...
                className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-[#0A66C2]/10 text-[#0A66C2] hover:bg-[#0A66C2] hover:text-white transition-all text-xs font-bold shadow-sm cursor-pointer group min-h-[38px]"
              >
            ...
            {social.youtube?.enabled !== false && social.youtube?.url && (
              <a
                ...
                className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-[#FF0000]/10 text-[#FF0000] hover:bg-[#FF0000] hover:text-white transition-all text-xs font-bold shadow-sm cursor-pointer group min-h-[38px]"
              >
            ...
            {social.tiktok?.enabled !== false && social.tiktok?.url && (
              <a
                ...
                className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-black/10 text-black hover:bg-black hover:text-white transition-all text-xs font-bold shadow-sm cursor-pointer group min-h-[38px]"
              >
      ...
      {/* Main Floating Call Now Button Bar */}
      <div className="relative group flex items-center">
        ...
        <a
          ...
          className="relative z-10 flex items-center gap-2 sm:gap-3 pl-2 sm:pl-3 pr-3.5 sm:pr-5 py-2 sm:py-3 rounded-full bg-gradient-to-r from-[#006e21] via-[#007a25] to-[#005a1b] text-white shadow-[0_10px_30px_rgba(0,110,33,0.45)] hover:shadow-[0_14px_38px_rgba(0,110,33,0.6)] border-2 border-white/90 hover:scale-[1.03] active:scale-[0.98] transition-all duration-200 cursor-pointer overflow-hidden min-h-[44px]"
        >
          {/* Circular Phone Icon Badge with Acoustic Glow */}
          <div className="relative flex items-center justify-center w-8 h-8 sm:w-11 sm:h-11 rounded-full bg-white text-[#006e21] shadow-md shrink-0 group-hover:rotate-12 transition-transform duration-300">
            <PhoneCall className="w-4 h-4 sm:w-6 sm:h-6 text-[#006e21] animate-bounce" />
            <span className="absolute -top-0.5 -right-0.5 w-2.5 h-2.5 sm:w-3 sm:h-3 bg-lime-400 rounded-full border-2 border-white animate-ping" />
            <span className="absolute -top-0.5 -right-0.5 w-2.5 h-2.5 sm:w-3 sm:h-3 bg-lime-400 rounded-full border-2 border-white" />
          </div>

          {/* Typography: Prominent Label & Large Phone Number */}
          <div className="flex flex-col items-start leading-tight">
            <div className="flex items-center gap-1 sm:gap-1.5">
              <span className="text-[9px] sm:text-[11px] font-black uppercase tracking-wider text-lime-200 drop-shadow-sm">
                {phoneLabel}
              </span>
              <span className="w-1.5 h-1.5 rounded-full bg-lime-300 animate-pulse" />
              <span className="text-[9px] font-bold text-white/80 hidden md:inline">
                {phoneSubtitle}
              </span>
            </div>
            <span className="text-sm sm:text-lg font-black tracking-wide text-white font-mono drop-shadow">
              {rawPhone}
            </span>
          </div>
        </a>
        ...
        {activeSocialCount > 0 && (
          <button
            ...
            className="relative z-10 -ml-2.5 sm:-ml-3 w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-[#00356a] text-white border-2 border-white shadow-md flex items-center justify-center hover:bg-[#002244] hover:scale-110 active:scale-95 transition-all cursor-pointer min-h-[28px]"
            aria-label="Toggle Social Channels"
          >
            {showSocialDock ? (
              <ChevronDown className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-lime-300" />
            ) : (
              <MessageCircle className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-lime-300" />
            )}
          </button>
        )}
      </div>
